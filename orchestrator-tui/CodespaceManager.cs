...
public static class CodespaceManager
{
    ...
    private const int STATE_POLL_INTERVAL_SEC = 20;
    private const int STATE_POLL_MAX_DURATION_MIN = 10;
    private const int PROVISIONING_MAX_WAIT_MIN = 5;
                ...
                case "Provisioning": case "Creating": case "Starting":
                    if (provisioningStopwatch == null) {
                        ...
                        if (provisioningMinutes >= PROVISIONING_MAX_WAIT_MIN) {
                            AnsiConsole.MarkupLine($"[red]  ✗ STUCK >{PROVISIONING_MAX_WAIT_MIN} min. DELETING![/]");
    ...
    public static async Task UploadAllBotData(TokenEntry token, string codespaceName) {
        ...
        string repoNameLower = token.Repo.ToLower();
        string remoteRepoRoot=$"/workspaces/{repoNameLower}"; int filesUploaded=0, botsSkipped=0;
