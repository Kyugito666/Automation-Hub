...
public static class BillingManager
{
    ...
    // ... (sisa kode GetBillingInfo dan DisplayBilling tetap sama seperti sebelumnya) ...
    public static async Task<BillingInfo> GetBillingInfo(TokenEntry token)
    {
        if (string.IsNullOrEmpty(token.Username)) {
            AnsiConsole.MarkupLine("[red]Username tidak diketahui.[/]");
        ...
        for (int attempt = 1; attempt <= MAX_BILLING_RETRIES; attempt++) {
            ...
            try {
                ...
                if (response.IsSuccessStatusCode) {
                    ...
                    if (report?.UsageItems == null) { AnsiConsole.MarkupLine("[yellow]WARN: Invalid billing format.[/]"); return new BillingInfo { IsQuotaOk = false, Error = "Invalid JSON format" }; }
                    ...
                    if (attempt < MAX_BILLING_RETRIES) {
                        ...
                        if (TokenManager.RotateProxyForToken(token)) { await Task.Delay(2000); continue; }
                        else { AnsiConsole.MarkupLine("[red]   Gagal rotasi proxy.[/]"); return new BillingInfo { IsQuotaOk = false, Error = PersistentProxyError }; }
                    } else { AnsiConsole.MarkupLine($"[red]   Gagal setelah {MAX_BILLING_RETRIES} attempts (407).[/]"); return new BillingInfo { IsQuotaOk = false, Error = PersistentProxyError }; }
                }
                else {
                    var error = await response.Content.ReadAsStringAsync();
                    AnsiConsole.MarkupLine($"[red]FAIL ({response.StatusCode})[/]"); AnsiConsole.MarkupLine($"[yellow]   WARN: Gagal API billing.[/]"); AnsiConsole.MarkupLine($"[dim]   {error.Split('\n').FirstOrDefault()}[/]");
                    return new BillingInfo { IsQuotaOk = false, Error = response.StatusCode.ToString() };
                }
            }
            catch (Exception ex) when (ex is HttpRequestException || ex is TaskCanceledException) {
                AnsiConsole.MarkupLine($"[red]FAIL (Connection Error)[/]"); AnsiConsole.MarkupLine($"[yellow]   Exception: {ex.Message.Split('\n').FirstOrDefault()}[/]");
                if (attempt < MAX_BILLING_RETRIES) {
                    AnsiConsole.MarkupLine($"[yellow]   Rotating proxy...[/]");
                    if (TokenManager.RotateProxyForToken(token)) { await Task.Delay(3000); continue; }
                    else { AnsiConsole.MarkupLine("[red]   Gagal rotasi proxy.[/]"); return new BillingInfo { IsQuotaOk = false, Error = PersistentProxyError }; }
                } else { AnsiConsole.MarkupLine($"[red]   Gagal koneksi setelah {MAX_BILLING_RETRIES} attempts.[/]"); return new BillingInfo { IsQuotaOk = false, Error = PersistentProxyError }; }
            }
            catch (Exception ex) { AnsiConsole.MarkupLine($"[red]FAIL (Unexpected Error)[/]"); AnsiConsole.MarkupLine($"[red]   Exception: {ex.Message}[/]"); return new BillingInfo { IsQuotaOk = false, Error = $"Unexpected: {ex.GetType().Name}" }; }
        }
        AnsiConsole.MarkupLine("[red]Billing Check: Gagal (fallback).[/]"); return new BillingInfo { IsQuotaOk = false, Error = PersistentProxyError };
    }
     ...
     public static void DisplayBilling(BillingInfo billing, string username) {
         if (!string.IsNullOrEmpty(billing.Error) && !billing.IsQuotaOk) {
              string reason = billing.Error == PersistentProxyError ? "[magenta]Persistent Proxy/Network Error[/]" :
              ...
              if (billing.Error == PersistentProxyError) AnsiConsole.MarkupLine($"   [yellow]Attempting automatic IP Authorization...[/]");
              else AnsiConsole.MarkupLine($"   [yellow]Assuming quota is insufficient.[/]");
...
// ... (Struct BillingInfo, BillingReport, UsageItem tidak berubah) ...
