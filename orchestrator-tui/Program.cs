...
internal static class Program
{
    ...
    public static async Task Main(string[] args)
    {
        Console.CancelKeyPress += (sender, e) => {
            ...
            if (_interactiveCts != null && !_interactiveCts.IsCancellationRequested) {
                AnsiConsole.MarkupLine("\n[yellow]Ctrl+C: Stopping interactive session...[/]");
                _interactiveCts.Cancel();
            } else if (!_mainCts.IsCancellationRequested) {
                AnsiConsole.MarkupLine("\n[red]Ctrl+C: Shutting down main loop...[/]");
                _mainCts.Cancel();
            } else { AnsiConsole.MarkupLine("[yellow]Shutdown already in progress...[/]"); }
        };
        try {
        ...
        catch (Exception ex) { AnsiConsole.MarkupLine("\n[red]FATAL ERROR in Main:[/]"); AnsiConsole.WriteException(ex); }
    ...
    private static async Task RunInteractiveMenuAsync(CancellationToken cancellationToken)
    {
        while (!cancellationToken.IsCancellationRequested) {
            ...
            var selection = AnsiConsole.Prompt( new SelectionPrompt<string>()
                    .Title("\n[bold white]MAIN MENU[/]").PageSize(7).WrapAround(true)
                    .AddChoices(new[] {
            ...
            catch (OperationCanceledException) when (cancellationToken.IsCancellationRequested) { AnsiConsole.MarkupLine("\n[yellow]Main cancellation requested.[/]"); return; }
    ...
    // ... (ShowSetupMenuAsync, ShowLocalMenuAsync, ShowAttachMenuAsync tetap sama) ...
    ...
    private static async Task RunOrchestratorLoopAsync(CancellationToken cancellationToken)
    {
        ...
        while (!cancellationToken.IsCancellationRequested) // Cek cancel di awal loop
        {
            ...
            try
            {
                cancellationToken.ThrowIfCancellationRequested(); // Cek sebelum billing
                AnsiConsole.MarkupLine("Checking billing quota...");
                var billingInfo = await BillingManager.GetBillingInfo(currentToken);
                cancellationToken.ThrowIfCancellationRequested(); // Cek setelah billing
                ...
                if (!billingInfo.IsQuotaOk)
                {
                    ...
                    {
                        AnsiConsole.MarkupLine("[magenta]Persistent proxy error during billing check.[/]");
                        AnsiConsole.MarkupLine("[magenta]Attempting auto IP Auth...[/]");
                        bool ipAuthSuccess = await ProxyManager.RunIpAuthorizationOnlyAsync(cancellationToken);
                        cancellationToken.ThrowIfCancellationRequested(); // Cek setelah IP Auth

                        if (ipAuthSuccess) {
                            AnsiConsole.MarkupLine("[green]IP Auth OK.[/]");
                            AnsiConsole.MarkupLine("[magenta]Attempting auto Proxy Test & Save...[/]");
                            bool testSuccess = await ProxyManager.RunProxyTestAndSaveAsync(cancellationToken);
                            cancellationToken.ThrowIfCancellationRequested(); // Cek setelah Test Proxy

                            if(testSuccess) {
                                AnsiConsole.MarkupLine("[green]Proxy Test OK. Reloading proxy list...[/]");
                                // === PERBAIKAN: Panggil ReloadProxyListAndReassign ===
                                TokenManager.ReloadProxyListAndReassign();
                                // === AKHIR PERBAIKAN ===
                                currentToken = TokenManager.GetCurrentToken(); // Re-get token (proxy mungkin berubah)
                                AnsiConsole.MarkupLine("[yellow]Retrying operation with same token, refreshed proxies...[/]");
                                await Task.Delay(5000, cancellationToken);
                                continue; // Coba lagi dari awal loop
                            } else {
                                AnsiConsole.MarkupLine("[red]Auto Proxy Test failed.[/]");
                                AnsiConsole.MarkupLine("[yellow]Proceeding with token rotation...[/]");
                            }
                        } else {
                            AnsiConsole.MarkupLine("[red]Auto IP Auth failed.[/]");
                            AnsiConsole.MarkupLine("[yellow]Proceeding with token rotation...[/]");
                        }
                    }

                    // Rotasi token jika kuota habis ATAU recovery proxy gagal
                    AnsiConsole.MarkupLine("[yellow]⚠ Rotating token (low quota or unrecoverable error)...[/]");
                    if (!string.IsNullOrEmpty(activeCodespace)) {
                         try { await CodespaceManager.DeleteCodespace(currentToken, activeCodespace); }
                         catch { /* Ignore delete error during rotation */ }
                ...
                // Billing OK, lanjut
                cancellationToken.ThrowIfCancellationRequested(); // Cek sebelum ensure codespace
                ...
                cancellationToken.ThrowIfCancellationRequested(); // Cek setelah ensure codespace
                ...
                if (isNewOrRecreatedCodespace) { AnsiConsole.MarkupLine($"[green]✓ New/Recreated CS activated: {activeCodespace}[/]"); }
                else { AnsiConsole.MarkupLine($"[green]✓ Reusing existing CS: {activeCodespace}[/]"); }
                consecutiveErrors = 0; // Reset error jika sampai sini

                AnsiConsole.MarkupLine($"\n[yellow]⏱ Keep-Alive:[/] Sleeping for {KeepAliveInterval.TotalHours:F1} hours...[/]");
                AnsiConsole.MarkupLine($"[dim]Next check: {DateTime.Now.Add(KeepAliveInterval):yyyy-MM-dd HH:mm:ss}[/]");
                // Delay dengan check cancellation
                await Task.Delay(KeepAliveInterval, cancellationToken);
                // Langsung cek setelah delay selesai
                ...
                if (string.IsNullOrEmpty(activeCodespace)) { AnsiConsole.MarkupLine("[yellow]⚠ No active CS after sleep.[/]"); continue; }

                AnsiConsole.MarkupLine("\n[yellow]⏱ Keep-Alive Check:[/] Verifying health...[/]");
                if (!await CodespaceManager.CheckHealthWithRetry(currentToken, activeCodespace)) {
                    ...
                    AnsiConsole.MarkupLine("[green]✓ Keep-Alive: Health OK.[/]");
                    try {
                        ...
                        AnsiConsole.MarkupLine("[green]✓ Keep-alive script triggered.[/]");
                    } catch (Exception ex) {
                        AnsiConsole.MarkupLine($"[yellow]⚠ Keep-alive trigger failed: {ex.Message.Split('\n').FirstOrDefault()}[/]");
                        AnsiConsole.MarkupLine("[yellow]Assuming CS issue, marking for recreation...[/]");
                        currentState.ActiveCodespaceName = null; TokenManager.SaveState(currentState); continue;
                    }
                }
            }
                ...
                if (consecutiveErrors >= MAX_CONSECUTIVE_ERRORS) {
                    ...
                    currentToken = TokenManager.SwitchToNextToken(); // Rotasi token
                    consecutiveErrors = 0;
                    AnsiConsole.MarkupLine("[cyan]Waiting 30s before retry...[/]");
                    // Cek cancel selama delay error
                    try { await Task.Delay(30000, cancellationToken); } catch (OperationCanceledException) { break; } // Keluar while jika di-cancel pas nunggu
                } else {
                    ...
                    // Cek cancel selama delay error
                    try { await Task.Delay(ErrorRetryDelay, cancellationToken); } catch (OperationCanceledException) { break; } // Keluar while jika di-cancel pas nunggu
                }
            }
        } // End while
     ...
     private static void Pause(string message, CancellationToken cancellationToken)
    {
       if (cancellationToken.IsCancellationRequested) return;
        ...
        try { while (!Console.KeyAvailable) { if (cancellationToken.IsCancellationRequested) return; System.Threading.Thread.Sleep(100); } Console.ReadKey(true); }
        catch (InvalidOperationException) { AnsiConsole.MarkupLine("[yellow] (Auto-continuing...)[/]"); System.Threading.Thread.Sleep(2000); }
    }
