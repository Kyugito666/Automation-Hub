...
namespace Orchestrator
{
    internal static class Program
    {
            ...
            while (!cancellationToken.IsCancellationRequested) {
                ...
                try {
                    switch (choice) {
                        ...
                        case "0": AnsiConsole.MarkupLine("Exiting..."); if (!_isShuttingDown) _mainCts.Cancel(); return;
                    }
                }
        ...
        private static async Task ShowRemoteShellAsync(CancellationToken mainCancellationToken)
        {
            if (mainCancellationToken.IsCancellationRequested || _mainCts.IsCancellationRequested) return;
            AnsiConsole.Clear();
            AnsiConsole.Write(new FigletText("Shell").Color(Color.Magenta1));
        ...
        // === FUNGSI INI DITARUH DI SINI, DI DALAM CLASS PROGRAM ===
        /// <summary>
        /// Provides access to the main application cancellation token.
        /// </summary>
        /// <returns>The CancellationToken from the main CancellationTokenSource.</returns>
        public static CancellationToken GetMainCancellationToken() => _mainCts.Token;
        // === AKHIR FUNGSI ===

    } // Kurung kurawal penutup class Program
