...
public static class InteractiveProxyRunner
{
    ...
    {
        AnsiConsole.MarkupLine($"[bold cyan]=== Interactive Proxy Mode: {{bot.Name}} ===[/]"); // C# Interpolation
        ...
        {
            AnsiConsole.MarkupLine($"[red]Bot path not found: {{botPath}}[/]"); // C# Interpolation
            return;
        }
        ...
        var inputsFile = Path.Combine(InputsDir, $"{bot.Name}.json"); // C# Interpolation
        ...
        AnsiConsole.MarkupLine($"[green]✓ Inputs saved to: {{inputsFile}}[/]"); // C# Interpolation
    ...
    private static async Task<Dictionary<string, string>?> RunBotInCaptureMode(string botPath, BotEntry bot)
    {
        ...
        {
             AnsiConsole.MarkupLine($"[red]✗ Tidak bisa menemukan command utama (npm start atau file .js/.py) untuk {{bot.Name}}.[/]"); // C# Interpolation
             return null;
        }
        ...
        {
            executor = "python";
            args = $"-u \"{pyWrapperFileName}\" {originalArgs}"; // C# Interpolation
        }
        else if (bot.Type == "javascript")
        {
            executor = "node";
            ...
            if (originalExecutor == "npm" && originalArgs == "start")
            {
                ...
                if (!string.IsNullOrEmpty(mainJs))
                {
                    ...
                    AnsiConsole.MarkupLine($"[grey]Detected 'npm start', assuming target script is '{{targetScriptArg}}'[/]"); // C# Interpolation
                }
                else
                {
                    AnsiConsole.MarkupLine($"[red]✗ Tidak bisa otomatis mendeteksi file JS utama untuk 'npm start' di {{bot.Name}}. Capture mungkin gagal. Mencoba 'index.js'.[/]"); // C# Interpolation
            ...
            args = $"\"{jsWrapperFileName}\" \"{targetScriptArg}\""; // C# Interpolation
        }
        else
        {
             AnsiConsole.MarkupLine($"[red]✗ Tipe bot tidak dikenal: {{bot.Type}}[/]"); // C# Interpolation
             return null;
        }
        ...
        {
            AnsiConsole.MarkupLine($"[red]Execution error: {{ex.Message}}[/]"); // C# Interpolation
            AnsiConsole.MarkupLine($"[dim]Command: {{executor}} {{args}}[/]"); // C# Interpolation
            AnsiConsole.MarkupLine($"[dim]Working Dir: {{absoluteBotPath}}[/]"); // C# Interpolation
        }
        ...
        if (File.Exists(inputCapturePath))
        {
            try
            {
                ...
                try {
                     ...
                     AnsiConsole.MarkupLine($"[yellow]Warning: Could not parse capture file (might be incomplete due to cancel): {{jsonEx.Message}}[/]"); // C# Interpolation
            ...
            {
                AnsiConsole.MarkupLine($"[yellow]Warning: Could not process/delete capture file: {{ioEx.Message}}[/]"); // C# Interpolation
                 inputs = new Dictionary<string, string>();
            }
        } else if (!Program.InteractiveBotCancelled) {
             AnsiConsole.MarkupLine($"[yellow]Warning: Input capture file not found at {{inputCapturePath}} after normal execution.[/]"); // C# Interpolation
        } else {
    ...
    // === METHOD PYTHON WRAPPER (FIXED ESCAPING) ===
...
def handle_signal(sig, frame):
    ...
    # Let finally block handle saving
    sys.exit(128 + sig)
...
// --- Input Capture Setup ---
try {{
    const nullStream = new Writable({{ // JS object literal -> {{}}
