using System.Net;
using System.Security.Cryptography; // Tidak terpakai, bisa dihapus jika mau
using System.Text; // Tidak terpakai, bisa dihapus jika mau
...
{
    // === PERBAIKAN PATH ===
    private static readonly string ProjectRoot = GetProjectRoot(); // Root dari repo automation-hub
    private static readonly string ConfigRoot = Path.Combine(ProjectRoot, "config");
    private static readonly string ProxySyncRoot = Path.Combine(ProjectRoot, "proxysync"); // Path ke folder proxysync

    private static readonly string TokensPath = Path.Combine(ConfigRoot, "github_tokens.txt");
    // Path proxy sekarang menunjuk ke dalam folder proxysync
    ...
    private static string GetProjectRoot()
    {
        ...
        // Cari ke atas sampai ketemu folder yang punya 'config' dan '.gitignore'
        ...
        // Fallback jika tidak ketemu (seharusnya tidak terjadi di struktur project normal)
    ...
    public static void ReloadProxyListAndReassign() {
        ...
        _availableProxies.Clear();
        LoadProxyList(); // Fungsi ini yang akan load dari path baru
    ...
    private static void LoadProxyList() {
        // Logika prioritas sudah benar: Cek success_proxy.txt dulu, baru proxy.txt
        string proxyFileToLoad = File.Exists(SuccessProxyListPath) && new FileInfo(SuccessProxyListPath).Length > 0
            ...
            : File.Exists(FallbackProxyListPath) && new FileInfo(FallbackProxyListPath).Length > 0
                ...
                : ""; // Kosong jika keduanya tidak ada/kosong
        ...
        try {
            ...
            } else { // Berarti dari FallbackProxyListPath
    ...
    public static HttpClient CreateHttpClient(TokenEntry token) {
        ...
        if (!string.IsNullOrEmpty(token.Proxy)) {
            try {
                // === Logika parsing proxy tetap sama ===
                ...
                if (!string.IsNullOrEmpty(proxyUri.UserInfo)) {
                    ...
                    if (credentials.Length == 2) {
                        ...
                        webProxy.UseDefaultCredentials = false; // Penting diset false jika ada creds
                    } else {
                        AnsiConsole.MarkupLine($"[yellow] (Proxy credential format issue for {MaskProxy(token.Proxy)}? Trying without auth)[/]");
                        webProxy.UseDefaultCredentials = true; // Coba tanpa auth jika format salah
                     }
                } else {
                    webProxy.UseDefaultCredentials = true; // Pakai default jika tidak ada UserInfo
                }
        ...
        httpClient.DefaultRequestHeaders.Add("User-Agent", "Automation-Hub/4.1"); // Mungkin perlu update versi?
...
// Class TokenEntry dan TokenState tidak berubah
