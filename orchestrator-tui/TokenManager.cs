...
public static class TokenManager
{
    ...
    private static void LoadTokenCache() { if (!File.Exists(TokenCachePath)) return; try { var json = File.ReadAllText(TokenCachePath); _tokenCache = JsonSerializer.Deserialize<Dictionary<string, string>>(json) ?? new(); AnsiConsole.MarkupLine($"[dim]Loaded {_tokenCache.Count} usernames from cache[/]"); } catch (Exception ex) { /* Log warn */ _tokenCache = new(); } }
    public static void SaveTokenCache(Dictionary<string, string> cache) { try { _tokenCache = cache; var json = JsonSerializer.Serialize(_tokenCache, new JsonSerializerOptions { WriteIndented = true }); File.WriteAllText(TokenCachePath, json); } catch (Exception ex) { /* Log error */ } }
    private static void LoadTokens() { if (!File.Exists(TokensPath)) { /* Error */ return; } var lines = File.ReadAllLines(TokensPath); if (lines.Length < 3) { /* Error */ return; } var owner = lines[0].Trim(); var repo = lines[1].Trim(); var tokens = lines[2].Split(',', StringSplitOptions.RemoveEmptyEntries); _tokens = tokens.Select(t => new TokenEntry { Token = t.Trim(), Owner = owner, Repo = repo }).ToList(); AnsiConsole.MarkupLine($"[green]Loaded {_tokens.Count} tokens for {owner}/{repo}[/]"); }
    private static void LoadProxyList() { string proxyFileToLoad = File.Exists(SuccessProxyListPath) ? SuccessProxyListPath : MasterProxyListPath; if (File.Exists(proxyFileToLoad)) { try { _availableProxies = File.ReadAllLines(proxyFileToLoad).Select(l=>l.Trim()).Where(l => !string.IsNullOrWhiteSpace(l) && !l.StartsWith("#")).Distinct().ToList(); /* Log */ } catch (Exception ex) { /* Log err */ _availableProxies.Clear(); } } else { /* Log warn */ _availableProxies.Clear(); } }
    ...
    public static bool RotateProxyForToken(TokenEntry currentTokenEntry) { if (!_proxyPool.TryGetValue(currentTokenEntry.Token, out var pool) || pool.Count == 0) { /* Log reset */ if (!_availableProxies.Any()) return false; var s = _availableProxies.OrderBy(x => Guid.NewGuid()).ToList(); pool = new Queue<string>(s); _proxyPool[currentTokenEntry.Token] = pool; if (pool.Count > 0) { currentTokenEntry.Proxy = pool.Peek(); /* Log next */ return true; } else { return false; } } var nextProxy = pool.Dequeue(); currentTokenEntry.Proxy = nextProxy; AnsiConsole.MarkupLine($"[yellow]🔁 Proxy rotated to: {MaskProxy(nextProxy)} ({pool.Count} left)[/]"); return true; }
    private static void LoadState() { try { if (File.Exists(StatePath)) { var json = File.ReadAllText(StatePath); _state = JsonSerializer.Deserialize<TokenState>(json) ?? new TokenState(); } } catch (Exception ex) { /* Log err, reset */ _state = new TokenState(); } if (_state.CurrentIndex >= _tokens.Count) { _state.CurrentIndex = 0; } }
    public static TokenState GetState() => _state;
    public static void SaveState(TokenState state) { _state = state; var json = JsonSerializer.Serialize(_state, new JsonSerializerOptions { WriteIndented = true }); try { File.WriteAllText(StatePath, json); } catch (Exception ex) { /* Log err */ } }
    ...
    public static TokenEntry SwitchToNextToken() {
        ...
        _state.ActiveCodespaceName = null; SaveState(_state); var current = _tokens[_state.CurrentIndex];
    ...
    public static void ShowStatus() { if (!_tokens.Any()) { /* Log */ return; } /* Log Owner/Repo/CS */ var table = new Table().Title("Tokens Status").Expand(); table.AddColumn("Idx"); table.AddColumn("Token"); table.AddColumn("User"); table.AddColumn("Proxy"); table.AddColumn("Active"); for (int i = 0; i < _tokens.Count; i++) { var t = _tokens[i]; var act = i == _state.CurrentIndex ? "[g✓[/]" : ""; var tokD = MaskToken(t.Token); var proxD = !string.IsNullOrEmpty(t.Proxy) ? MaskProxy(t.Proxy) : "[g none[/]"; table.AddRow((i+1).ToString(), tokD, t.Username ?? "[g???[/]", proxD, act); } AnsiConsole.Write(table); AnsiConsole.MarkupLine($"[dim] Proxies available: {_availableProxies.Count}[/]"); }
}
