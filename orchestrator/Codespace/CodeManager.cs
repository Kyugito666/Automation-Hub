    ...
    public static class CodeManager
    {
                ...
                while (stopwatch.Elapsed.TotalMinutes < STATE_POLL_MAX_DURATION_MIN) {
                    ...
                    switch (codespace.State) {
                        case "Available":
                            ...
                            bool scriptResult = await CodeActions.RunStartupScriptAndStreamLogs(token, codespace.Name, cancellationToken);
                            
                            if (scriptResult) {
                                // Sukses (termasuk sukses sebagian)
                                AnsiConsole.MarkupLine("[green]✓ Health OK (script success). Ready.[/]"); 
                                stopwatch.Stop(); 
                                return codespace.Name; 
                            }
                            else { 
                                // Gagal (HANYA jika ProxySync Gagal)
                                var lastState = await CodeActions.GetCodespaceState(token, codespace.Name); 
                                AnsiConsole.MarkupLine($"[red]Health failed (FATAL script error, e.g. ProxySync) & state '{lastState?.EscapeMarkup() ?? "Unknown"}'. Deleting...[/]"); 
                                await CodeActions.DeleteCodespace(token, codespace.Name); 
                                codespace = null; 
                                break; 
                            }
        ...
        private static async Task<string> CreateNewCodespace(TokenEntry token, string repoFullName, CancellationToken cancellationToken)
        {
            ...
            try {
                ...
                bool scriptResult = await CodeActions.RunStartupScriptAndStreamLogs(token, newName, cancellationToken);
                ...
                if (scriptResult) {
                    ...
                    // Gagal (HANYA jika ProxySync Gagal)
                    throw new Exception("Remote script health check failed after create (FATAL, e.g. ProxySync). Check remote logs.");
                }
                // === AKHIR PERBAIKAN ===
