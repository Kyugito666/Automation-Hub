        ...
        internal static async Task<bool> RunStartupScriptAndStreamLogs(TokenEntry token, string codespaceName, CancellationToken cancellationToken)
        {
            ...
            try 
            {
                Func<string, bool> logCallback = (line) => {
                    ...
                    // === PERBAIKAN LOGIC "SETRES": ===
                    // Kita anggap sukses kalo udah sampe summary
                    if(line.Contains("DEPLOYMENT SUMMARY")) {
                        scriptSuccess = true; 
                    }
                    // TAPI, kalo ProxySync gagal, itu FATAL
                    if(line.Contains("ERROR: ProxySync failed")) {
                        scriptSuccess = false; 
                    }
                    // Kita NGGAK peduli lagi sama "ERROR: Bot deployment failed"
                ...
                // Cek final
                if (stdout.Contains("ERROR: ProxySync failed")) {
                    scriptSuccess = false;
                }
                
                // Kalo exit code-nya 1 (karena 5 bot gagal), GhService bakal throw error.
                // Tapi kita udah nangkep 'scriptSuccess = true' dari log "DEPLOYMENT SUMMARY".
                // Jadi kita return 'true' kalo udah dapet summary.
                
                AnsiConsole.MarkupLine($"[green]✓ Remote script finished (stream closed).[/]");
                ...
                // Ini blok 'catch' kalo exit code-nya 1
                AnsiConsole.MarkupLine($"\n[yellow]Remote script exited with error (Exit Code 1).[/]"); 
                AnsiConsole.MarkupLine($"[dim]   Error: {ex.Message.Split('\n').FirstOrDefault()?.EscapeMarkup()}[/]");
                
                // === PERBAIKAN LOGIC "SETRES": ===
                // Kalo 'scriptSuccess' udah true (dari "DEPLOYMENT SUMMARY"),
                // kita tetep return true. Kita anggap sukses.
                if (scriptSuccess)
                {
                    AnsiConsole.MarkupLine("[green]✓ Deployment Summary was detected. Accepting partial success.[/green]");
                    return true;
                }
                
                // Kalo 'scriptSuccess' masih false (misal ProxySync gagal), baru kita return false.
                AnsiConsole.MarkupLine("[red]✗ No Deployment Summary detected. Script FAILED.[/red]");
                return false; 
                // === AKHIR PERBAIKAN LOGIC ===
            }
        }
