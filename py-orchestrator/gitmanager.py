...
from rich.progress import Progress, SpinnerColumn, BarColumn, TextColumn, PercentageColumn
...
async def validate_all_tokens():
    ...
    with Progress(
        ...
        task = progress.add_task("[green]Memvalidasi token...[/]", total=len(tokens))

        for entry in tokens:
            token_display = TokenManager.mask_token(entry.token)
            progress.update(task, description=f"[green]Memvalidasi:[/] {token_display}")
...
async def invite_collaborators():
    ...
    with Progress(
        ...
        task = progress.add_task("[green]Mengirim undangan...[/]", total=len(users_to_invite))
        
        async with TokenManager.create_http_client(main_token_entry) as client:
            for user in users_to_invite:
                progress.update(task, description=f"[green]Mengundang:[/] [yellow]@{user.username}[/]")
...
async def accept_invitations():
    ...
    with Progress(
        ...
        task = progress.add_task("[green]Menerima undangan...[/]", total=len(tokens))

        for entry in tokens:
            token_display = TokenManager.mask_token(entry.token)
            progress.update(task, description=f"[green]Mengecek:[/] {entry.username or token_display}")
